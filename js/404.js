...
var squareBoxes = function(){
  $(function(){

    var sectionW = $('.draw_canvas').width();

    var boxW = sectionW/20;

    var rows = 40;

    var inc = Math.round((sectionW/boxW))*rows;


    for(var x=1; x<inc+1; x++){

      var code='<div class="card_contianer" id="' + x +'"><div class="card"><div class="front face"></div><div class="back face center"></div></div></div>';

      $('.draw_canvas').append(code);
  ...
  //Button Events
     document.onkeydown = function(evt) {
        ...
        switch (evt.keyCode) {
          ...
          case 68:
            evt.preventDefault();
            $('#mo_to_draw').toggleClass('active');
            break;
        }
      };


    $('#draw, #mo_to_draw').on('click',function(){

      $(this).toggleClass('active');

    });

    $('.card_contianer').bind('mouseenter',function(){

        if($('#draw').hasClass('active') && $('#mo_to_draw').hasClass('active') ){

          $(this).toggleClass('zoom_effect');
        ...
        $(this).find('.face.back').css({'background-color': hue});

    });


    $('.card_contianer').bind('click',function(){
    ...
    $("#clear").bind('click', function(){
      ...
      $('.zoom_effect').toggleClass('zoom_effect');

    });

    $('#print').bind('click', function(){

      var get_clicked = $('.zoom_effect').length;

      var list1=$('.zoom_effect');

      var list2=[];

      for(var i=0; i<get_clicked; i++){


        var id =$(list1[i]).attr('id');

        list2.push(id);

      }

      var print_list = '"' + list2.join('","') + '"';
  ...
  //setup for drawing
    function draw(){
      ...
      $('#draw, #mo_to_draw').addClass('active');
